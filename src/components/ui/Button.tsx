...
export function Button({ href, variant = 'primary', withArrow, className = '', children, ...props }: ButtonProps) {
  const baseClass = "inline-flex items-center gap-2.5 px-7 py-[15px] rounded-full text-sm tracking-[0.04em] font-medium transition-all duration-350 ease-[cubic-bezier(0.2,0.7,0.2,1)] border border-transparent leading-none cursor-pointer group";
  
  const variants = {
    primary: "bg-accent-gold text-[#1a1a1a] hover:bg-[#b89456] hover:-translate-y-[1px] hover:shadow-[0_12px_28px_rgba(201,168,106,0.32)]",
  ...
  const content = (
    <>
      {children}
      {withArrow && <span className="transition-transform duration-350 ease-out group-hover:translate-x-1">→</span>}
    </>
  );
